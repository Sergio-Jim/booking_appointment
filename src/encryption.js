...
// encryption request to get 
export const encryptRequest = (requestBody, publicPem) => {
  // Generate a random AES key (128-bit) and IV
  const aesKey = crypto.randomBytes(16);
  const iv = crypto.randomBytes(12); // GCM IV is typically 12 bytes

  // Encrypt the request body using AES-128-GCM
  const cipher = crypto.createCipheriv("aes-128-gcm", aesKey, iv);
  const encryptedFlowData = Buffer.concat([
    cipher.update(JSON.stringify(requestBody), "utf-8"),
    cipher.final(),
    cipher.getAuthTag(), // Append authentication tag for GCM mode
  ]).toString("base64");
